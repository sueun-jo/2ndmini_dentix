...
void FirstScreen::on_loginButton_clicked(){
    ...
    if (!client->connectToServer("127.0.0.1", 54321)) //서버 연결 요청
    {
        QMessageBox::warning(this, "서버 연결 실패", "서버에 연결할 수 없습니다.");
        return;
    }

    emit loginRequest(name, pw);
    qDebug() << "login request signal emit";
